<x-app-layout>
  <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">Detail Tamu</h2></x-slot>
  <div class="py-6">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white p-5 rounded shadow space-y-3">
        ...
        <div><span class="text-gray-500">Tanggal/Waktu:</span> {{ $guest->visit_date?->format('d/m/Y') }} {{ $guest->visit_time?->format('H:i') }}</div>
        <div><span class="text-gray-500">Kontak:</span> {{ $guest->phone }} {{ $guest->email ? ' / '.$guest->email : '' }}</div>
